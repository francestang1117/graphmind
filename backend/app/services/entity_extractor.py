...
class EntityExtractor:
    ...
    def _extract_technical_terms(self, text: str) -> list[Entity]:
        ...
        for term, label in TECH_TERMS.items():
            ...
            for match in re.finditer(pattern, text, re.IGNORECASE):
                entities.append(
                    self._entity(match.group(0), label, match.start(), match.end(), 0.9, "rule")
                )
        return entities
    ...
    def _entity(
        ...
        display = CANONICAL_TERMS.get(clean.lower(), clean)
        return Entity(
            text=display,
            label=label,
            start=start,
    ...
    def _normalize(self, text: str) -> str:
        ...
        return CANONICAL_TERMS.get(compact.lower(), compact)
