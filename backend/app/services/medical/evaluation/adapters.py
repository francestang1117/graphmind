...
def evaluate_terminology(case: EvaluationCase, _dataset_root: Path) -> dict[str, Any]:
    """Exercise the privacy-bounded local query builder without contacting PubMed."""
    ...
    return {
        ...
        "error_code": None,
        "raw_fragments_in_query": [
            value for value in fragments if value and value.casefold() in normalized_query.casefold()
        ],
    }


def evaluate_insight_safety(case: EvaluationCase, _dataset_root: Path) -> dict[str, Any]:
    """Run citation, claim-support, and safety validators over synthetic evidence."""
    ...
    try:
        ...
        return {
            ...
            "error_codes": ["report_schema_invalid"],
            "validator_errors": [str(error.get("type", "validation")) for error in exc.errors()],
        }
...
def evaluate_literature_matching(case: EvaluationCase, dataset_root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "reason_complete": all(bool(card.match_reasons) for card in cards),
